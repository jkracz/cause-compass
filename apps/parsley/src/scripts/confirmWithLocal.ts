...
async function processOrgsWithLocalLLM() {
    try {
        ...
        // Log summary
        logger.info(`Processed ${results.length} organizations using local Ollama model`);
        ...
        logger.error("Error in processOrgsWithLocalLLM:", error instanceof Error ? error.message : String(error));
        throw error;
    } finally {
...
async function processOrganization(org: TaxExemptOrganization) {
    ...
    try {
        ...
        if (response.data && response.data.message && response.data.message.content) {
            ...
            logger.warn("Invalid response from Ollama chat API, falling back to generate API");
            // Fall through to the generate API fallback
        }
    } catch (error: unknown) {
        logger.warn(
    ...
    try {
        ...
        logger.error(`Error calling Ollama API: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
