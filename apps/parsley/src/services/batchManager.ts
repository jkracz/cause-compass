...
const DEFAULT_BATCH_SIZE = process.env.BATCH_SIZE
...
export class BatchManager {
      ...
      if (job) {
        ...
        if (job.status === "processing") {
          ...
          if (updatedJob.status === "downloading") {
            logger.info(
              `Job ${updatedJob.id} is now ready for download, processing immediately`,
            );
            ...
            logger.info(
              `Job ${updatedJob.id} has failed, starting a new batch job`,
            );
            await this.startNewBatchJob();
            return;
          }
        ...
        switch (job.status) {
          ...
          case "failed":
            ...
            logger.info(
              `Job ${job.id} is ${job.status}, starting a new batch job`,
            );
            await this.startNewBatchJob();
            return;
          ...
          default:
            ...
            logger.info(
              `Job ${job.id} has status ${job.status}, no action needed`,
            );
            return;
        }
      }
  ...
  private async generateBatchFile(
    ...
    try {
      ...
      if (!fs.existsSync(inputFilePath)) {
        throw new Error(
          `Generated file does not exist at path: ${inputFilePath}`,
        );
      }
  ...
  private async checkBatchStatus(
    job: BatchJob,
  ): Promise<"downloading" | "failed" | "processing"> {
    try {
      if (!job.batchId) {
      ...
      switch (batchStatus.status) {
        ...
        case "finalizing":
          ...
          logger.info(
            `Batch job ${job.id} is still processing (status: ${batchStatus.status})`,
          );
          break;
        case "expired":
        case "cancelling":
        case "cancelled":
          logger.error(
            `Batch job ${job.id} ended with status: ${batchStatus.status}`,
          );
          await this.updateBatchJob(job.id, {
            status: "failed",
      ...
      if (error instanceof Error && error.message.includes("429")) {
        logger.warn(
          `Rate limit hit checking batch status for job ${job.id}, will retry later`,
        );
      } else {
  ...
  private async downloadAndProcessResults(job: BatchJob): Promise<void> {
    try {
      ...
      const { processedCount, errorCount } = await processBatchResponseFile(
        outputFilePath,
        processedDir,
        {
          batchJobId: job.id,
      ...
      await this.updateBatchJob(job.id, {
        ...
        processedCount,
        error:
          errorCount > 0 ? `Processed with ${errorCount} errors` : undefined,
      });
  ...
  private async startNewBatchJob(): Promise<boolean> {
    ...
    logger.info(
      `Created new batch job ${job.id} for ${orgsToProcess.length} organizations`,
    );
