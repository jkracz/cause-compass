...
function SpotlightLargeCard({
  ...
  return (
    <article
      ...
      className="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-[2rem] border border-[var(--rule)] bg-white p-8 shadow-[0_24px_60px_-36px_rgba(91,75,158,0.4)] transition-all duration-500 hover:border-[var(--accent)]/40 hover:shadow-[0_32px_80px_-30px_rgba(200,38,110,0.4)] sm:p-10 lg:col-span-7"
    >
      <div className="flex items-start justify-between">
        <span className="inline-flex items-baseline gap-1.5 rounded-full border border-[var(--accent)]/30 bg-[var(--accent-soft)] px-3 py-1 text-[10px] font-semibold tracking-[0.32em] text-[var(--accent)] uppercase">
          Pick<span className="font-mono tracking-tight normal-case">№01</span>
        </span>
        <ArrowUpRight className="h-5 w-5 text-[var(--ink-mute)] transition-all group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-[var(--accent)]" />
      </div>
      ...
      <span
        aria-hidden
        className="absolute right-0 bottom-0 left-0 h-px origin-left scale-x-0 bg-gradient-to-r from-transparent via-[var(--accent)] to-transparent transition-transform duration-700 group-hover:scale-x-100"
      />
    </article>
  );
}
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <ArrowUpRight className="h-4 w-4 text-[var(--ink-mute)] transition-all group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-[var(--accent)]" />
        </div>
