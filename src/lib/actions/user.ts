...
export async function getLikedOrganizationsAction() {
  const cookieStore = await cookies();
  const userId = cookieStore.get("userId")?.value;
  ...
  try {
    const likedOrgIds = await getLikedOrganizations(userId);
    return likedOrgIds;
  } catch (error) {
    console.error("Error getting liked organizations:", error);
    return [];
  }
}
