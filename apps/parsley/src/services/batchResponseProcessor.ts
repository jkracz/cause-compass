  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        if (!customId) {
          logger.warn(
            `Missing custom_id in file ${fileName}, line: ${line.substring(0, 100)}...`,
          );
          errorCount++;
          continue;
        }
    ...
    for (const org of orgs) {
      ...
      const parsedContent = WebsiteConfirmationSchema.parse(
        JSON.parse(content),
      );
      if (parsedContent.hasCorrectWebsite) {
        const correctCrawlItems: CrawlItem[] | undefined =
          org.confirmationCrawlItems?.filter((ci: CrawlItem) => {
        ...
        orgUpdates.push({
          ...
          geographicFocus:
            parsedContent.organizationGeographicFocus ?? undefined,
