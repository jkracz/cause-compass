        ...
        async requestHandler({ request, page, enqueueLinks, addRequests }) {
          ...
          try {
            ...
            await Promise.all([
              ...
              options?.addLinks || options?.addLinks === undefined
                ? enqueueLinks()
                : Promise.resolve(),
            ...
            if (
              ...
              criticalErrors.some((msg) => error.message.includes(msg))
            ) {
...
export const getCrawlDataAsArray = async (
  datasetName?: string,
  worker?: boolean,
): Promise<CrawlItem[]> => {
  let config = undefined;
  ...
  const dataset = datasetName
    ? await Dataset.open(datasetName, { config })
    : Dataset;
