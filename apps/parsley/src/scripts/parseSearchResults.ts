      ...
      threadLimit(
        () =>
          new Promise<void>((resolve, reject) => {
            ...
            const worker = fork(
              "./src/workers/confirmationCrawlWorker.ts",
              [],
              {
                env: {
                  ...process.env,
                  ORG_DATA: JSON.stringify(org),
                  WORKER_INDEX: `${org.name}`,
                },
              },
            );
            workers.push(worker);
            ...
            worker.on("message", (message) => {
              if (typeof message === "object" && "error" in message) {
                ...
                logger.error(`Worker error for org ${org.name}:`, {
                  ...
                  orgData: (message as { error: unknown; orgData?: unknown })
                    .orgData,
                });
                reject(new Error(message.error as string));
              } else if (
                typeof message === "object" &&
                "confirmationCrawlItems" in message
              ) {
                cleanup();
