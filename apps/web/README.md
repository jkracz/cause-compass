...
Cause Compass uses guest sessions plus Better Auth sign-in through Convex.

- Anonymous visitors get a browser-scoped `guestId` cookie automatically.
- Signed-in users authenticate with Better Auth using Google OAuth or email/password.
- Convex verifies Better Auth JWTs generated by the Convex Better Auth component.
...
The local web origin must match the Convex `SITE_URL` environment variable. For normal local development that should be `http://localhost:3000`.
